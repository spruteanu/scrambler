...
public abstract class Combinations<T> extends ConstantData<T> {
    ...
    public static <T> Data<T[]> dataOf(Class<T> dataType, Data<T>... original) {
        return new DataArrayCombinations<T>(original, dataType);
    }
    ...
    public static <T> Data<List<T>> dataOf(List<Data<T>> original) {
        return new DataListCombinations<T>(original, original.size());
    }
    ...
    static class ArrayCombinations<T> extends Combinations<T[]> {
        ...
        ArrayCombinations(T[] original) {
            ...
            this.dataType = (Class<T>) original.getClass().getComponentType();
        }

        @SuppressWarnings("unchecked")
        T[] create() {
            return (T[]) Array.newInstance(dataType, count);
        }
    ...
    static class DataArrayCombinations<T> extends Combinations<T[]> {
        private Data<T>[] original;
        private Class<T> dataType;

        DataArrayCombinations(Data<T>[] original, Class<T> dataType) {
            super(original.length);
            this.original = original;
    ...
    static class DataListCombinations<T> extends Combinations<List<T>> {
        private List<Data<T>> original;

        @SuppressWarnings("unchecked")
        DataListCombinations(List<Data<T>> original, int count) {
            super(count);
            this.original = original;
        }
