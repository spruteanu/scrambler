 ...
 */
@CompileStatic
class Random extends RandomRange {
    private static
    final String NOT_SUPPORTED_RANGE_TYPE_MSG = "Not supported range method for provided class type: %s, range of [%s, %s]"
    ...
    static <T> Property<T> of(String propertyName, T minimum, T maximum) {
    ...
    static <T> Property<T> of(String propertyName, Class<T> clazzType, T minimum, T maximum) {
...
//        }
    ...
    static <T> Property<T> of(String propertyName, Collection<T> collection) {
        return new RandomListElement<T>(propertyName, new ArrayList<T>(collection))
    }

    static <T> Property<T> of(String propertyName, List<T> values) {
