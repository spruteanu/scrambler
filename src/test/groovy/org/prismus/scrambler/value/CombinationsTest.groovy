...
class CombinationsTest extends Specification {
    ...
    void 'verify values and list combinations'() {
        def combinations = Combinations.valuesOf(Integer, NumberScrambler.random(1, 100), NumberScrambler.random(200, 400), NumberScrambler.random(-100, -10))

        expect: 'check array values generation'
        combinations.next().length > 0

        and: 'check list generation'
        Combinations.of([1, 2, 3, 4, 5]).next().size() > 0

        and: 'check list values generation'
        Combinations.valuesOf(
